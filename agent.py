...
def llm_node(state: MessagesState):
    """
    Takes the previous 10 messages from state and returns AI response.
    """
    ...
    # Ensure system message exists only once at the beginning
    if not any(isinstance(m, SystemMessage) for m in last_messages):
        last_messages = [system_message] + last_messages
